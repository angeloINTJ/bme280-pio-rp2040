...
void read_cal(){uint8_t b[26];sl();d5();cl();d5();wb(0xEC);wb(0x88);sl();d5();ch();d5();sh();d5();sl();d5();cl();d5();wb(0xED);for(int i=0;i<26;i++)b[i]=rb(i==25);sl();d5();ch();d5();sh();d5();
...
void setup(){
    ...
    read_cal();Serial.print("Cal T1=");Serial.print(cal.T1);Serial.print(" P1=");Serial.println(cal.P1);
    ...
    for(int n=0;n<15;n++){
        ...
        int32_t tf;float t=cT(aT,tf);float p=cP(aP,tf);
        ...
        int32_t tf2;float gt=cT(gaT,tf2);float gp=cP(gaP,tf2);
